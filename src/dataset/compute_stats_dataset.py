...
def print_stats(dataset: str, stats: tuple) -> None:
...
def get_stats_image(image_scene_graph: dict,
                    num_outlier_bboxes: int,
                    num_bboxes_with_phrases: int,
                    bbox_counter_dict: dict,
                    outlier_bbox_counter_dict: dict) -> tuple[int, int]:
...
def compute_stats_for_csv_file(dataset: str, path_csv_file: str) -> tuple:
    ...
    with open(path_csv_file) as csv_file:
        ...
        for row in tqdm(csv_reader, total=num_images):
            ...
            # compute new number for num_outlier_bboxes and num_bboxes_with_phrases based on current image
            # also increase the counter dicts
...
def compute_and_print_stats_for_csv_files(csv_files_dict):
    ...
    for dataset, path_csv_file in csv_files_dict.items():
        ...
        num_images, \
            ...
            outlier_bbox_counter_dict = stats
    ...
    stats = total_num_images, \
        total_num_ignored_images, \
        total_num_bboxes, \
        total_num_bboxes_with_phrases, \
        total_num_outlier_bboxes, \
        total_bbox_with_phrases_counter_dict, \
        total_outlier_bbox_counter_dict

    print_stats(dataset="Total", stats=stats)
