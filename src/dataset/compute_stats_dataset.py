...
def print_stats(dataset, stats):
    num_images, num_ignored_images, num_bboxes, num_bboxes_with_phrases, num_outlier_bboxes, outlier_bbox_counter_dict = stats
    ...
    print(f"\t{num_bboxes_with_phrases:,} bboxes in total have corresponding phrases")
    print(f"\t{num_outlier_bboxes:,} bboxes with phrases have 'outlier' anatomical region names")
    print(f"\t-> {(num_outlier_bboxes/num_bboxes_with_phrases) * 100:.2f}% of bboxes with phrases are outliers")

    print("\n\tCounts of 'outlier' bboxes:")
    for bbox_name, count in sorted(outlier_bbox_counter_dict.items(), key=lambda k_v: k_v[1], reverse=True):
        print(f"\t{bbox_name}: {count}")
    print()


def get_stats_image(image_scene_graph: dict, num_outlier_bboxes: int, num_bboxes_with_phrases: int, outlier_bbox_counter_dict: dict) -> tuple[int, int]:
    for attribute in image_scene_graph["attributes"]:
        ...
        # they are considered outliers and are counted here
...
def compute_stats_for_csv_file(dataset, path_csv_file):
    ...
    outlier_bbox_counter_dict = defaultdict(int)
    ...
    with open(path_csv_file) as csv_file:
        ...
        for row in tqdm(csv_reader, total=num_images):
            ...
            num_outlier_bboxes, num_bboxes_with_phrases = get_stats_image(image_scene_graph, num_outlier_bboxes, num_bboxes_with_phrases, outlier_bbox_counter_dict)

            # for each image, there are 36 bboxes for 36 anatomical regions
            num_bboxes += 36

    stats = num_images, num_ignored_images, num_bboxes, num_bboxes_with_phrases, num_outlier_bboxes, outlier_bbox_counter_dict
    print_stats(dataset, stats)

    return stats


def compute_stats_for_csv_files(csv_files_dict):
    total_num_images = 0
    ...
    stats = total_num_images, total_num_ignored_images, total_num_bboxes, total_num_bboxes_with_phrases, total_num_outlier_bboxes, total_outlier_bbox_counter_dict
...
def main():
    ...
    compute_stats_for_csv_files(csv_files_dict)
